...
func (a *Auth) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		_ = a.Store.Pool.QueryRow(r.Context(), `
		  INSERT INTO users (name, username, created_at, updated_at)
		  VALUES ($1, $2, now(), now())
		  RETURNING id
	...
	_ = a.Store.Pool.QueryRow(r.Context(), `
	  INSERT INTO accounts (user_id, provider, provider_account_id, access_token, refresh_token, expires_at, provider_data, created_at, updated_at)
...
func (a *Auth) Logout(w http.ResponseWriter, r *http.Request) {
	if sid, err := r.Cookie("ink_sid"); err == nil {
		_, _ = a.Store.Pool.Exec(r.Context(), `DELETE FROM sessions WHERE session_token=$1`, sid.Value)
	}
	c := &http.Cookie{
		Name:     "ink_sid",
		Value:    "",
...
func (a *Auth) OAuthCallback(w http.ResponseWriter, r *http.Request) {
	...
	// upsert user
	var userID uuid.UUID
	err = a.Store.Pool.QueryRow(ctx, `SELECT user_id FROM accounts WHERE provider=$1 AND provider_account_id=$2`, provider, providerAccountID).Scan(&userID)
	if err != nil {
		if email != "" {
			_ = a.Store.Pool.QueryRow(ctx, `SELECT id FROM users WHERE email=$1`, email).Scan(&userID)
		}
		if userID == uuid.Nil {
			_ = a.Store.Pool.QueryRow(ctx, `INSERT INTO users (email, name, username, avatar_url, created_at, updated_at) VALUES ($1,$2,$3,$4, now(), now()) RETURNING id`, email, name, fallbackUsername(name), avatar).Scan(&userID)
		}
	}
	...
	_, _ = a.Store.Pool.Exec(ctx, `INSERT INTO accounts (user_id, provider, provider_account_id, access_token, refresh_token, expires_at, provider_data, created_at, updated_at)
	 VALUES ($1,$2,$3,$4,$5,$6,$7, now(), now())
	...
	sessionExpiry := time.Now().Add(30 * 24 * time.Hour)
	if _, err := a.Store.Pool.Exec(ctx, `INSERT INTO sessions (session_token, user_id, expires_at, created_at, last_seen_at) VALUES ($1,$2,$3, now(), now())`, sessionToken, userID, sessionExpiry); err != nil {
    log.Printf("[auth] failed to insert session: %v", err)
	}
...
func (a *Auth) UnlinkAccount(w http.ResponseWriter, r *http.Request) {
	...
	var count int
	if err := a.Store.Pool.QueryRow(r.Context(), `SELECT COUNT(*) FROM accounts WHERE user_id = $1`, sd.UserID).Scan(&count); err == nil {
		if count <= 1 {
	...
	if _, err := a.Store.Pool.Exec(r.Context(), `DELETE FROM accounts WHERE user_id = $1 AND provider = $2`, sd.UserID, in.Provider); err != nil {
...
func (a *Auth) ListAccounts(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := a.Store.Pool.Query(r.Context(), `SELECT provider, provider_account_id, provider_data, created_at, updated_at FROM accounts WHERE user_id=$1 ORDER BY updated_at DESC`, sd.UserID)
	if err != nil {
